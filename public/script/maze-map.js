...
function getNeighbors(current, unvisited) {
  ...
  var row = current.row;
  var col = current.col;
  if (row != 0 && unvisited[row - 1][col]) neighbors.push(map[row - 1][col]);
  if (row != rows - 1 && unvisited[row + 1][col])
    neighbors.push(map[row + 1][col]);
  if (col != 0 && unvisited[row][col - 1]) neighbors.push(map[row][col - 1]);
...
function animate() {
  ...
  animation = setInterval(function () {
    ...
    if (current.w) ctx.fillRect(current.x, current.y, wallSize, floorSize);
