...
function clear() {
  if (game != null) window.clearInterval(game);
  if (animation != null) window.clearInterval(animation);
}
...
function update() {
  ...
  if (player.x < 0 || player.x > WIDTH - player.size) player.x -= player.velX;
  else if (player.velX > 0) {
    left = Math.floor((player.x - player.speed) / floorSize);
    if (map[top][left].e || map[bottom][left].e) player.x -= player.velX;
    else if (horizontalWall(top, bottom, left)) player.x -= player.velX;
    left = Math.floor(player.x / floorSize);
  } else if (player.velX < 0) {
    right = Math.floor((player.x + floorSize) / floorSize);
    if (map[top][right].w || map[bottom][right].w) player.x -= player.velX;
    else if (horizontalWall(top, bottom, right)) player.x -= player.velX;
  ...
  if (player.y < 0 || player.y > HEIGHT - player.size * 2)
    ...
    if (map[bottom][left].n || map[bottom][right].n) player.y -= player.velY;
    else if (verticalWall(left, right, bottom)) player.y -= player.velY;
  } else if (player.velY < 0) {
    top = Math.floor(player.y / floorSize);
    if (map[top][left].s || map[top][right].s) player.y -= player.velY;
    else if (verticalWall(left, right, top)) player.y -= player.velY;
  }
...
function horizontalWall(top, bottom, dir) {
  return (
    (map[bottom][dir].y > player.y &&
      map[bottom][dir].y < player.y + floorSize) ||
...
function verticalWall(left, right, dir) {
  return (
    (map[dir][right].x > player.x &&
      map[dir][right].x < player.x + floorSize) ||
