...
function getNeighbors(e, l) {
  var r = [],
    ...
    o = e.col;
  return (
    0 != a && l[a - 1][o] && r.push(map[a - 1][o]),
    a != rows - 1 && l[a + 1][o] && r.push(map[a + 1][o]),
    0 != o && l[a][o - 1] && r.push(map[a][o - 1]),
    o != cols - 1 && l[a][o + 1] && r.push(map[a][o + 1]),
    r
  );
}
...
function animate() {
  ...
  window.clearInterval(game),
    null != animation && window.clearInterval(animation),
    ...
    (animation = setInterval(function () {
      (current = l[r]),
        ...
        current.w && ctx.fillRect(current.x, current.y, wallSize, floorSize),
        r == l.length - 1 && window.clearInterval(animation);
    }, 1));
}
function play() {
...
function clear() {
  null != game && window.clearInterval(game),
    null != animation && window.clearInterval(animation);
}
function main() {
  ...
  for (var e = 0; rows > e; e++)
    for (var l = 0; cols > l; l++)
      e == exit[0] &&
        ...
        map[e][l].w &&
          ctx.fillRect(map[e][l].x, map[e][l].y, wallSize, floorSize);
}
...
function update() {
  if (0 == player.velX || 0 == player.velY) {
    ...
    (player.x += player.velX),
      player.x < 0 || player.x > WIDTH - player.size
        ? (player.x -= player.velX)
        : player.velX > 0
        ? ((r = Math.floor((player.x - player.speed) / floorSize)),
          map[e][r].e || map[l][r].e
            ? (player.x -= player.velX)
            : horizontalWall(e, l, r) && (player.x -= player.velX),
          (r = Math.floor(player.x / floorSize)))
        : player.velX < 0 &&
          ((a = Math.floor((player.x + floorSize) / floorSize)),
          map[e][a].w || map[l][a].w
            ? (player.x -= player.velX)
            : horizontalWall(e, l, a) && (player.x -= player.velX),
          (a = Math.floor((player.x + floorSize - player.speed) / floorSize))),
      (player.y += player.velY),
      player.y < 0 || player.y > HEIGHT - 2 * player.size
        ? (player.y -= player.velY)
        : player.velY > 0
        ? ((l = Math.floor((player.y + floorSize - player.speed) / floorSize)),
          map[l][r].n || map[l][a].n
            ? (player.y -= player.velY)
            : verticalWall(r, a, l) && (player.y -= player.velY))
        : player.velY < 0 &&
          ((e = Math.floor(player.y / floorSize)),
          map[e][r].s || map[e][a].s
            ? (player.y -= player.velY)
            : verticalWall(r, a, e) && (player.y -= player.velY));
...
function horizontalWall(e, l, r) {
  return (
    (map[l][r].y > player.y && map[l][r].y < player.y + floorSize) ||
    (map[e][r].y > player.y && map[e][r].y < player.y + floorSize)
  );
}
function verticalWall(e, l, r) {
  return (
    (map[r][l].x > player.x && map[r][l].x < player.x + floorSize) ||
    (map[r][e].x > player.x && map[r][e].x < player.x + floorSize)
  );
}
function end() {
